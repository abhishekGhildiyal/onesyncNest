...
import { Store } from '../../modules/users/entities/store.entity'; // Verify path
...
import { ProductList } from '../../modules/products/entities/product-list.entity';
...
import { reduceSoldQuantityForPackages } from './reduce-package-qty.helper';

export const markSoldInventory = async (
  orderId: number,
  soldDate: Date,
  storeId: number,
  userId: number,
  ...
  try {
    ...
    const products = await PackageBrand.findAll({
    ...
    for (const [_, group] of groups.entries()) {
      ...
      for (const entry of variantEntries.filter(
        ...
        const inventoryIds = [
          ...new Set(variantsToSell.map((v) => v.inventoryId).filter(Boolean)),
        ];
      ...
      for (const [brandId, brandQty] of brandMap.entries()) {
        await reduceSoldQuantityForPackages({
          productId,
          storeId,
          size,
          soldQty: brandQty,
      ...
      if (validItems.length > 0) {
        ...
        for (const [productId, shopifyIds] of Object.entries(
          ...
          if (allDeletedOrNotFound) {
            // Desync Web items
            const webItems = await Inventory.findAll({
              where: {
                productId,
                publishedScope: 'web',
                storeId,
